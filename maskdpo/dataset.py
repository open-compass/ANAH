...
def tokenize(pair: str,
    ...
    rejected = tokenizer.apply_chat_template(
        ...
        tokenize=False,
        add_generation_prompt=False)
    chosen_sents, chosen_types = pair['chosen_item']["sents"], pair['chosen_item']['type']
    rejected_sents, rejected_types = pair['rejected_item']["sents"], pair['rejected_item']['type']
    ...
    if is_reward:
        ...
        copy_chosen_labels = copy.deepcopy(chosen_labels)
        start = prompt_len
        for sent, hallu_type in zip(chosen_sents, chosen_types):
            length = len(tokenizer.encode(sent, add_special_tokens=False))
            if hallu_type == "hallucination":
                chosen_labels[start:start + length] = [-100] * length
            start += length
        rejected_labels = [-100] * prompt_len + copy.deepcopy(
            rejected_ids[prompt_len:])
        copy_rejected_labels = copy.deepcopy(rejected_labels)
        start = prompt_len
        for sent, hallu_type in zip(rejected_sents, rejected_types):
            length = len(tokenizer.encode(sent, add_special_tokens=False))
            if hallu_type == "no_hallucination":
                rejected_labels[start:start + length] = [-100] * length
            start += length
    try:
        assert len(chosen_ids) == len(chosen_labels)
        assert len(rejected_ids) == len(rejected_labels)
    except:
        print(f"The question of {pair['prompt'][0]['content']} has error! Id length is not equal to label length. chosen_ids: {len(chosen_ids)}, chosen_labels: {len(chosen_labels)}, rejected_ids: {len(rejected_ids)}, rejected_labels: {len(rejected_labels)}")
        chosen_labels = copy_chosen_labels
        rejected_labels = copy_rejected_labels
